...
Providers can advertise multiple capabilities from a single class — Firecrawl, Tavily, Exa, Parallel, and Kagi all implement both search and extract. Brave Search, DDGS, xAI, and SearXNG are search-only; SearXNG has a documented "pair me with an extract provider" workflow.
...
- **`plugins/web/kagi/`** — paid Bearer-auth HTTP provider using Kagi's v1 `/search` and `/extract` APIs.
